...
/**
 * @typedef {object} FormatInterface
 * @property {{ red: (text: string) => string, gray: (text: string) => string }} chalk
 * @property {(text: string, level?: number) => string} indent
 */
...
export default async function * customReporter (source) {
  const format = {
    logSymbols: { success: '✓', info: 'ℹ' },
    chalk: {
      gray: /** @param {string} text */ (text) => chalk.gray(text),
      red: /** @param {string} text */ (text) => chalk.red(text)
    },
    italic: /** @param {string} text */ (text) => chalk.italic(text),
    indent: /** @param {string} text @param {number} level */ (text, level = 1) => text.split('\n').map(/** @param {string} line */ line => '  '.repeat(level) + line).join('\n')
  };
